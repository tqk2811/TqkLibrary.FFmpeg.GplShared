...
# OVERLAY: pin FFmpeg to an exact commit for reproducible/consistent release builds.
# Set GIT_COMMIT in addins/<ver>.sh (e.g. 9.0.sh) or via the GIT_COMMIT_OVERRIDE env.
# The clone still uses --branch=$GIT_BRANCH to fetch the branch history; we then detach
# onto the pinned commit so the build is immune to the release branch advancing mid-run.
GIT_COMMIT="${GIT_COMMIT:-}"
GIT_COMMIT="${GIT_COMMIT_OVERRIDE:-$GIT_COMMIT}"
if [[ -n "$GIT_COMMIT" ]]; then
    GIT_CHECKOUT_LINE="git checkout --detach '$GIT_COMMIT'"
else
    GIT_CHECKOUT_LINE=":"
fi
